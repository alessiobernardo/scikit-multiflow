...
class DriftDetectionMethodClassifier(BaseSKMObject, ClassifierMixin):
    """ Drift Detection Method Classifier
        ...
        classifier :
            Classifier as the basis for the concept drift classifier
        ...
        >>> from skmultiflow.drift_detection import DriftDetectionMethodClassifier
        >>> from skmultiflow.drift_detection import ADWINChangeDetector
        >>> from skmultiflow.evaluation import EvaluatePrequential
        >>> drift_detection_method_classifier = DriftDetectionMethodClassifier(drift_detector=ADWINChangeDetector(), classifier=NaiveBayes())
        ...
        >>> # Evaluate the model
        >>> evaluator = EvaluatePrequential(show_plot=True, max_samples=data_stream.n_samples)
        >>> evaluator.evaluate(stream=data_stream, model=drift_detection_method_classifier, model_names=["NB_ADWIN"])
    ...
    def partial_fit(self, X, y, classes=None, sample_weight=None):
        ...
        self.drift_detection_method.add_element(input_value=add)
        ...
        if self.ddm_level == self.DDM_WARNING_LEVEL:
            ...
            if self.new_classifier_reset:
                ...
                self.new_classifier_reset = False
                # print("New warning at instance: ", self.n_instance)

            self.new_classifier.partial_fit(X, y)

        elif self.ddm_level == self.DDM_OUTCONTROL_LEVEL:
            self.change_detected += 1
            self.classifier = None
            self.classifier = self.new_classifier
            print("Change at instance: ", self.n_instance)
